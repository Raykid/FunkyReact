...
const regHTTPPath:RegExp = /^https?:/;
/**
 * 判断url是否是HTTP远程路径
 *
 * @author Raykid
...
{
    return url != null && regHTTPPath.test(url);
}
...
{
    if (url == null) throw new Error("url不能为空");
    var oriParams:any = getQueryParams(url);
    var targetParams:any = Object.assign(oriParams, params);
