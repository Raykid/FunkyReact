 ...
 */
export function formatTimestamp(timestamp:number, format:string="yyyy-MM-dd hh:mm:ss.S", transform?:(type:string, value:number)=>number|string):string
{
 ...
 */
export function formatUTCTimestamp(timestamp:number, format:string="yyyy-MM-dd hh:mm:ss.S", transform?:(type:string, value:number)=>number|string):string
{
    const date:Date = new Date(timestamp);
    return formatFromData({
 ...
 * @returns {string}
 */
export function formatDuration(duration:number, format:string="d hh:mm:ss.S", transform?:(type:string, value:number)=>number|string):string
{
    const data:DateData = {
...
function formatFromData(data:DateData, format:string, transform?:(type:string, value:number)=>number|string):string
{
    // 统一替换的不包含毫秒数
    ...
    for(let key in dict)
    {
        let value:number|string = dict[key];
        // 负数不替换
        if(value >= 0)
        {
            ...
            if(reg.test(format))
            {
                ...
                if(transform)
                {
                    const newValue:number|string = transform(key, value);
    ...
    {
        let value:number|string = data.millisecond;
        // 如果有转换器，调用之
        if(transform)
        {
            const newValue:number|string = transform("S+", value);
            if(newValue && newValue != null)
            {
                value = newValue;
            }
